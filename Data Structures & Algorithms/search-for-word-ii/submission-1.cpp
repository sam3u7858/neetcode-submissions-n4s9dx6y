...
// By Gemini NEETCODE Hint:
//你可以這樣最佳化：

// 在 TrieNode 紀錄子節點個數：新增一個變數 int nodesInSubtree。

// 插入單字時：路徑上的每個節點 nodesInSubtree++。

// 找到單字後（重點）：

// 當你發現 word != "" 並收集後，可以寫一個輔助函式從 root 到該節點的路徑上，將所有節點的 nodesInSubtree 減 1。

// DFS 剪枝：在 dfs 檢查條件中加入 if (nextNode->nodesInSubtree <= 0) return;。
